...
| 优先级 | 条件                      | 时间取值   | 地点列                | 刊名列              |
| --- | ----------------------- | ------ | ------------------ | ---------------- |
| 1   | `create_y/m/d` 有值       | 创建时间   | 填入 `location`      | 填入 `publication`（若 location 为空则 publication 加括号） |
| 3   | 创建仅有年，刊载至少有年+月，且创建年=刊载年 | 改用刊载时间 | 填入 `location`（加括号） | 填入 `publication` |
