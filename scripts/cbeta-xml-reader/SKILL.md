...
### 6. 全文输出（extract_article_fulltext.py）
...
- 篇名 + 题注（第 1–2 行）
- `## 目錄` + 目录树
- 目录与正文之间三行空白（Markdown 渲染后合为一行，源码中保留三行）
...
**编目录回写：** 提取时若使用 `--catalog` 模式，自动将原始 CJK 字符数写入编目录 JSON 的 `字数` 字段，并刷新编目录 MD。MD 中显示为：

```
- 1. 佛學概論（2.1 万字） （1930 年 1 月，在閩南佛學院編述）
```

**计算方法：** 统计全文 `md_lines` 中除 `## 目錄` 至下一 `## ` 标题区间外的所有 CJK 字符（0x4E00–0x9FFF, 0x3400–0x4DBF, 0xF900–0xFAFF），万字 = 原始字符数 / 10000，保留一位小数。
